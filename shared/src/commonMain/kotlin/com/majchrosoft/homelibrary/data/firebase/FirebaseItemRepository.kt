...
internal class FirebaseItemRepository(
    ...
    override fun observeMyLibrary(ownerId: String): Flow<List<Item>> =
        database.reference(FirebasePaths.userItems(ownerId))
            .valueEvents
            .map { snapshot ->
    ...
    override fun observeSharedCatalog(query: String?, limit: Int): Flow<List<Item>> =
        database.reference(FirebasePaths.sharedCatalog)
    ...
    override suspend fun getById(ownerId: String, itemId: String): Item? = runCatching {
        val snapshot = database.reference(FirebasePaths.userItem(ownerId, itemId))
            .valueEvents
            .first()
    ...
    override suspend fun add(item: Item): Result<Item> = runCatching {
    ...
    override suspend fun update(item: Item): Result<Unit> = runCatching {
        ...
        database.reference(FirebasePaths.userItem(item.ownerId, item.id))
            .setValue(Item.serializer(), toWrite)
    }

    override suspend fun delete(ownerId: String, itemId: String): Result<Unit> = runCatching {
 ...
 */
private fun Item.withId(id: String?, ownerId: String?): Item = copy(
    id = id ?: this.id,
