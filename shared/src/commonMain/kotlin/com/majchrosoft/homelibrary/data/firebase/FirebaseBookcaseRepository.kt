...
internal class FirebaseBookcaseRepository(
    ...
    override fun observeMine(ownerId: String): Flow<List<Bookcase>> =
        database.reference(FirebasePaths.userBookcases(ownerId))
            .valueEvents
            .map { snapshot ->
    ...
    override suspend fun getById(ownerId: String, bookcaseId: String): Bookcase? = runCatching {
        val snapshot = database.reference(FirebasePaths.userBookcase(ownerId, bookcaseId))
            .valueEvents
            .first()
    ...
    override suspend fun add(ownerId: String, bookcase: Bookcase): Result<Bookcase> = runCatching {
    ...
    override suspend fun update(ownerId: String, bookcase: Bookcase): Result<Unit> = runCatching {
        require(bookcase.id.isNotBlank()) { "Bookcase.id is required for update" }
        database.reference(FirebasePaths.userBookcase(ownerId, bookcase.id))
            .setValue(Bookcase.serializer(), bookcase)
    }

    override suspend fun delete(ownerId: String, bookcaseId: String): Result<Unit> = runCatching {
